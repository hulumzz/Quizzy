...
const pointerKey = (ownerId, sourceType, quizId) => ({ PK: `TEACHER#${ownerId}`, SK: `QUIZ_BANK_SOURCE#${sourceType}#${quizId}` });
...
const summary = (item) => ({ id: item.id, title: item.title, description: item.description || '', questionCount: item.questionCount, totalPoints: item.totalPoints, level: item.level, subject: quizSubjectById(item.subjectId), subjectId: item.subjectId, tags: item.tags || [], license: item.license, authorId: item.authorId, sourceType: item.sourceType || 'class', sourceQuizId: item.sourceQuizId, publishedAt: item.publishedAt, updatedAt: item.updatedAt });
...
export class QuizBankRepository {
  ...
  async publish({ ownerId, sourceType = 'class', classId, quizId, level, subjectId, tags, license, now = new Date().toISOString() }) {
    ...
    const pointer = await this.client.send(new GetCommand({ TableName: this.tableName, Key: pointerKey(ownerId, sourceType, quizId), ConsistentRead: true }));
    ...
    transactions.push({ Put: { TableName: this.tableName, Item: { ...pointerKey(ownerId, sourceType, quizId), entityType: 'QUIZ_BANK_SOURCE', ownerId, sourceType, quizId, catalogId: id, updatedAt: now }, ...(hasPointer ? {} : { ConditionExpression: 'attribute_not_exists(PK)' }) } });
    for (const key of listingKeys(level, subjectId, publishedAt, id)) transactions.push({ Put: { TableName: this.tableName, Item: { ...key, entityType: 'QUIZ_BANK_LISTING', id, title: item.title, description: item.description, questionCount: item.questionCount, totalPoints: item.totalPoints, level, subjectId, tags, license, authorId: ownerId, sourceType, sourceQuizId: quizId, publishedAt, updatedAt: now } } });
  ...
  async unpublish({ catalogId, ownerId }) {
    ...
    await this.client.send(new TransactWriteCommand({ TransactItems: [
      ...
      { Delete: { TableName: this.tableName, Key: pointerKey(ownerId, catalog.sourceType || 'class', catalog.sourceQuizId) } },
