...
export function createLiveQuizzesHandler({ authenticate, repository, answerQueue, logger = console }) {
  return async function liveQuizzesHandler(event, context = {}) {
    ...
    const createGeneral = path.match(/^\/general-quizzes\/([^/]+)\/live-sessions$/);
    const host = path.match(/^\/classes\/([^/]+)\/live-sessions\/([^/]+)$/);
    const hostGeneral = path.match(/^\/general-quizzes\/live-sessions\/([^/]+)$/);
    const action = path.match(/^\/classes\/([^/]+)\/live-sessions\/([^/]+)\/action$/);
    const actionGeneral = path.match(/^\/general-quizzes\/live-sessions\/([^/]+)\/action$/);
    ...
    try {
      ...
      if (!create && !createGeneral && !host && !hostGeneral && !action && !actionGeneral && !publicSession && !join && !answer && !result) return jsonResponse(404, { error: { code: 'NOT_FOUND', message: 'Endpoint tidak ditemukan.' } }, requestId);
      ...
      if (createGeneral && method === 'POST') return jsonResponse(201, { data: { session: await repository.create({ scope: 'general', quizId: validateQuizId(createGeneral[1]), ownerId: identity.uid, ...validateLiveSessionInput(event.body ? bodyOf(event) : {}) }) } }, requestId);
      if ((host || hostGeneral) && method === 'GET') return jsonResponse(200, { data: { session: await repository.hostStateById(host ? host[2] : hostGeneral[1], identity.uid) } }, requestId);
      if ((action || actionGeneral) && method === 'POST') return jsonResponse(200, { data: { session: await repository.advance({ sessionId: action ? action[2] : actionGeneral[1], ownerId: identity.uid, ...validateLiveAction(bodyOf(event)) }) } }, requestId);
