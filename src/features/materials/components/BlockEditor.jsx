...
export default function BlockEditor({ blocks, onChange, errors = {}, classId }) {
  ...
  return (
    <div className="qz-block-editor">
      {blocks.map((block, index) => (
        <section className="qz-editor-block" key={block.id}>
          ...
          {['link', 'youtube', 'image', 'file'].includes(block.type) ? <div className="qz-form-stack">
            {['image', 'file'].includes(block.type) ? <label className="qz-upload-control"><span>{block.type === 'image' ? 'Unggah gambar' : 'Unggah dokumen'}</span><input type="file" accept={block.type === 'image' ? 'image/jpeg,image/png,image/webp,image/gif' : '.pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt'} disabled={uploadingId === block.id} onChange={(event) => upload(index, block, event.target.files?.[0])} /><small>{uploadingId === block.id ? 'Mengunggah langsung ke Cloudinary...' : block.type === 'image' ? 'JPG, PNG, WebP, atau GIF · maks. 8 MB' : 'PDF, Office, atau TXT · maks. 15 MB'}</small></label> : null}
            {uploadErrors[block.id] ? <p className="qz-field__error" role="alert">{uploadErrors[block.id]}</p> : null}
            <Input label={['image', 'file'].includes(block.type) ? 'URL hasil unggahan atau URL HTTPS lain' : 'URL HTTPS'} type="url" value={block.url} onChange={(event) => update(index, { url: event.target.value })} placeholder={block.type === 'youtube' ? 'https://youtube.com/watch?v=…' : 'https://…'} maxLength={2048} error={errors[`blocks.${index}.url`]} /><Input label="Label" value={block.label} onChange={(event) => update(index, { label: event.target.value })} placeholder={block.type === 'image' ? 'Keterangan gambar' : 'Nama sumber atau file'} maxLength={160} error={errors[`blocks.${index}.label`]} />
          </div> : null}
        </section>
      ))}
