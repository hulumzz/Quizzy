...
import { Button, Card, Dialog, EmptyState, PageHeader, ProcessLoader, Skeleton, Textarea, buttonClassName } from '../components/ui';
...
export default function MaterialDiscussion({ role }) {
  ...
  const perform = async (request, after) => {
    ...
    try { await request(); await reload(); after?.(); return true; } catch (caught) { setActionError(discussionErrorMessage(caught)); return false; } finally { setBusy(false); }
  };
  const submit = () => perform(() => createDiscussion(classId, materialId, content.trim()), () => setContent(''));
  const confirmDelete = () => perform(() => deleteDiscussionMessage(classId, materialId, pendingDelete.discussionId, pendingDelete.replyId), () => setPendingDelete(null));
  ...
  return <div className="qz-dashboard qz-enter">
    ...
    {!configured ? <div className="qz-status-strip">Diskusi sedang disiapkan. Coba lagi beberapa saat.</div> : null}
    <Card className="qz-discussion-composer"><div><span className="qz-discussion-composer__icon"><DiscussionIcon size={22} /></span><div><h2>Mulai percakapan</h2><p>{role === 'teacher' ? 'Tambahkan arahan atau pertanyaan pemantik untuk kelas.' : 'Tanyakan bagian yang belum jelas atau bagikan hal yang kamu pahami.'}</p></div></div><Textarea label="Pesan" value={content} onChange={(event) => setContent(event.target.value)} placeholder="Tulis pesan yang jelas dan tetap sesuai topik..." maxLength={3000} hint={`${content.length}/3.000 karakter`} /><Button disabled={!configured || busy || content.trim().length < 2} onClick={submit}>{busy ? <><ProcessLoader size={16} label="Mengirim pesan" /> Memproses...</> : 'Kirim ke diskusi'}</Button></Card>
