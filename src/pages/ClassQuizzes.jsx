...
export default function ClassQuizzes({ role }) {
  ...
  const download = async (quiz) => { try { const payload = await exportQuiz(classId, quiz.id); const url = URL.createObjectURL(new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `${quiz.title.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '') || 'kuis'}-quizzy.json`; anchor.click(); URL.revokeObjectURL(url); } catch (error) { setState((current) => ({ ...current, error })); } };
