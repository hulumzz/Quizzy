...
function checkInSummary(checkIn) {
  if (!checkIn) return 'Belum ada check-in tercatat';
  const distance = Number.isFinite(checkIn.distanceMeters) ? `, jarak ${checkIn.distanceMeters} m dari titik` : '';
  return `${formatDateTime(checkIn.checkedInAt)}${distance}`;
}
...
export default function ClassAttendance({ role }) {
  ...
  useEffect(() => {
    ...
    Promise.all([getClass(classId, { signal: controller.signal }), role === 'teacher' ? listLearningSessions(classId, { signal: controller.signal }) : Promise.resolve([])])
      .then(([item, sessionItems]) => {
        setClassName(item.name);
        setSessions(sessionItems.filter((session) => session.status !== 'archived'));
      })
      .catch(() => {});
  ...
  const createSession = async (event) => {
    ...
    try {
      ...
      setForm(initialForm);
      setNotice('Sesi tersimpan sebagai draf. Mulai sesi saat siswa siap melakukan presensi.');
      await reload();
    } catch (caught) { setActionError(attendanceErrorMessage(caught)); } finally { setBusy(false); }
  };

  const changeStatus = async (attendanceId, nextStatus) => {
    setBusyId(attendanceId); setActionError(''); setNotice('');
    try {
    ...
    } catch (caught) { setActionError(attendanceErrorMessage(caught)); } finally { setBusyId(''); }
  };
