...
app.post('/api/ai/assist', async (c) => {
  ...
  if (!['material_draft', 'quiz_draft', 'assessment_feedback'].includes(task) || !context || context.length > 6_000 || instruction.length > 700) return bad('Tugas atau konteks AI tidak valid.');
  ...
  let response;
  try { response = await fetch(GROQ_URL, { method: 'POST', headers: { authorization: `Bearer ${c.env.GROQ_API_KEY}`, 'content-type': 'application/json' }, body: JSON.stringify({ model, messages: [{ role: 'user', content: prompt }], temperature: task === 'assessment_feedback' ? 0.4 : 0.55, max_completion_tokens: advanced ? 900 : 750, reasoning_effort: reasoningEffort, reasoning_format: 'hidden', ...(['quiz_draft', 'material_draft'].includes(task) ? { response_format: { type: 'json_object' } } : {}) }) }); } catch { return bad('Layanan AI sedang tidak dapat dihubungi.', 503, 'AI_UNAVAILABLE'); }
